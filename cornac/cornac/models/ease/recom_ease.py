...
class EASE(Recommender, ANNMixin):
    ...
    * Steck, H. (2019, May). "Embarrassingly shallow autoencoders for sparse data." \
    ...
    def fit(self, train_set, val_set=None):
        ...
        print("U shape: ", self.U.shape, "U nnz: ", self.U.nnz, "U dtype: ", self.U.dtype)
        G = self.U.T.dot(self.U).toarray()
        print("Computed U^T U in seconds...", time.time() - start_time)
        print("G shape: ", G.shape, "G nnz: ", np.count_nonzero(G), "G dtype: ", G.dtype)

        diag_indices = np.diag_indices(G.shape[0])
        print("Got diagonal indices in seconds...", time.time() - start_time)

        G[diag_indices] = G.diagonal() + self.lamb
        print("Added lambda to diagonal in seconds...", time.time() - start_time)

        P = np.linalg.inv(G)
        print("Inverted G matrix in seconds...", time.time() - start_time)
        print("P shape: ", P.shape, "P nnz: ", np.count_nonzero(P), "P dtype: ", P.dtype)

        B = P / (-np.diag(P))
        print("Normalized B matrix in seconds...", time.time() - start_time)
