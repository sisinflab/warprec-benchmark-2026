...
if __name__ == '__main__':
    ...
    our_logger = LifecycleLogger(f"results/{config['dataset']}-{config['algo_name']}.txt")
    our_logger.start_experiment(f"{config['algo_name']}")
    ...
    our_logger.start_preprocessing()
    ...
    def objective(trial):
        ...
        for train_index, val_index in splitter.split(train_set):
            ...
            our_logger.end_training()
            our_logger.start_evaluation()
            ...
            our_logger.end_evaluation()
            our_logger._log(f'Current {kpi_name}: {kpi:.4f}')
