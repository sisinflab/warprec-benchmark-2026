...
class RawDataReader(object):
    ...
    def get_data(self):
        ...
        # Add the following datasets with these structures
        # - ml-1m-custom: user_id,item_id,rating,timestamp
        # - ml-32m-custom: userId,movieId,rating,timestamp
        # - nf-100m-custom: user_id,item_id,rating,timestamp
        elif self.src == 'ml-1m-custom':
            df = pd.read_csv(f'{self.ds_path}ratings.csv')
            df.rename(columns={'user_id': self.uid_name, 'item_id': self.iid_name, 'rating': self.inter_name, 'timestamp': self.tid_name}, inplace=True)
        elif self.src == 'ml-32m-custom':
            df = pd.read_csv(f'{self.ds_path}ratings.csv')
            df.rename(columns={'userId': self.uid_name, 'movieId': self.iid_name, 'rating': self.inter_name, 'timestamp': self.tid_name}, inplace=True)
        elif self.src == 'nf-100m-custom':
            df = pd.read_csv(f'{self.ds_path}ratings_processed.csv')
            df.rename(columns={'user_id': self.uid_name, 'item_id': self.iid_name, 'rating': self.inter_name, 'timestamp': self.tid_name}, inplace=True)
