# WarpRec Reproducibility Package - RecSys 2026 Resources Track

This repository contains the reproducibility package for the experiments submitted to RecSys 2026 as a resources track paper. The package includes benchmark scripts for multiple recommendation frameworks across different datasets.
...
Two tables in the paper have a dedicated, self-contained sub-package under [`tables_reproducibility/`](tables_reproducibility/). Each sub-package ships its own README with environment setup, commands, and expected outputs:

- **Table 3 — Green AI profiling of WarpRec on NetflixPrize-100M.** Per-trial RAM/VRAM, energy and emissions for ItemKNN, EASE$^R$, NeuMF, LightGCN and SASRec, including pre-run estimates from the static memory estimator. See [`tables_reproducibility/table_3/README.md`](tables_reproducibility/table_3/README.md).
- **Table 5 — Cross-framework evaluation on MovieLens-1M.** 18-cell table where each framework (WarpRec, Elliot, RecBole) trains ItemKNN and LightGCN and scores the other frameworks' top-10 recommendations with its own metrics. Driven by a single `make all`. See [`tables_reproducibility/table_5/README.md`](tables_reproducibility/table_5/README.md).

The benchmark table is reproduced by running the SLURM scripts under [`slurm/`](slurm/) — see *Running Experiments with SLURM* below.
