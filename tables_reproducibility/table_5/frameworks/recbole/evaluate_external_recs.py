"""Evaluate externally-produced recommendation TSVs with RecBole's metrics.

Reads `results/recs/<framework>/<model>.tsv` (3 cols: user_id, item_id, score;
no header), maps the external token IDs to RecBole internal IDs, builds a
`DataStruct` containing `rec.items` + `rec.topk`, and runs the metrics pipeline
without retraining. Output: `results/recbole_runs/external/<framework>_metrics.csv`.
...
RECS_DIR = ROOT / "results" / "recs"
OUT_DIR = ROOT / "results" / "recbole_runs" / "external"
...
def build_topk_hits(rec_items, uid_list, uid2pos):
    rec_np = rec_items.numpy()
    n_users, k = rec_np.shape
    ...
    for row_idx, uid in enumerate(uid_list):
        ...
        for col_idx in range(k):
            if int(rec_np[row_idx, col_idx]) in positives:
...
def main():
    ...
    for framework in FRAMEWORKS:
        results = {}
        for model in MODELS:
            tsv = RECS_DIR / framework / f"{model}.tsv"
            if not tsv.is_file():
        ...
        out_path = OUT_DIR / f"{framework}_metrics.csv"
